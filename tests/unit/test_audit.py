...
async def make_audit_log(db_session, **overrides) -> AuditLog:
    log = AuditLog(
...
async def test_list_audit_logs_as_admin_succeeds(client, make_user, as_user, db_session):
    ...
    as_user(admin)
    await make_audit_log(db_session)
    await make_audit_log(db_session)
...
async def test_list_audit_logs_filters_by_entity_type(client, make_user, as_user, db_session):
    admin = await make_user(role=UserRole.ADMIN)
    as_user(admin)
    await make_audit_log(db_session, entity_type="incident")
    await make_audit_log(db_session, entity_type="mitigation")
...
async def test_list_audit_logs_filters_by_entity_id(client, make_user, as_user, db_session):
    ...
    await make_audit_log(db_session, entity_id=target_id)
    await make_audit_log(db_session, entity_id=uuid.uuid4())
...
async def test_list_audit_logs_filters_by_actor_id(client, make_user, as_user, db_session):
    ...
    as_user(admin)
    actor_id = uuid.uuid4()
    await make_audit_log(db_session, actor_id=actor_id)
    await make_audit_log(db_session, actor_id=uuid.uuid4())
...
async def test_list_audit_logs_orders_newest_first(client, make_user, as_user, db_session):
    ...
    older = await make_audit_log(
        db_session, created_at=datetime.now(timezone.utc) - timedelta(minutes=10)
    )
    newer = await make_audit_log(db_session, created_at=datetime.now(timezone.utc))
...
async def test_get_entity_audit_trail_as_responder_can_read(client, make_user, as_user, db_session):
    ...
    await make_audit_log(db_session, entity_id=entity_id)
