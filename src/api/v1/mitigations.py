...
async def create_mitigation(
    ...
    await apply_optimistic_update(
        db, incident_id, current_user.account_id, incident.version, {"status": IncidentStatus.MITIGATED}
    )
...
async def get_mitigation(
    ...
    return await get_mitigation_by_incident_or_404(db, incident_id, current_user.account_id)
...
async def clear_mitigation(
    ...
    mitigation = await get_mitigation_by_incident_or_404(db, incident_id, current_user.account_id)
    incident = await get_incident_or_404(db, incident_id, current_user.account_id)

    if incident.status == IncidentStatus.MITIGATED:
        await apply_optimistic_update(
            db, incident.id, current_user.account_id, incident.version, {"status": IncidentStatus.OPEN}
        )
